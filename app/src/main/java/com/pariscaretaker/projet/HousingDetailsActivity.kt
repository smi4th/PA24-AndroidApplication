...
class HousingDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        reserveButton.setOnClickListener {
            if (startDate != null && endDate != null) {
                Toast.makeText(this, "Réservation effectuée du ${SimpleDateFormat("dd-MM-yyyy", Locale.getDefault()).format(startDate!!.time)} au ${SimpleDateFormat("dd-MM-yyyy", Locale.getDefault()).format(endDate!!.time)} [FACTICE, J'AI PAS LA ROUTE]", Toast.LENGTH_LONG).show()
            } else {
    ...
    private fun markDateAsReserved(dateStr: String) {
        ...
        date?.let {
            ...
            calendarView.setEvents(reservedDates)
        }
    }
